...
class InMemoryCache:
    ...
    def set(self, range: tuple[int, int], data: bytes) -> None:
        ...
        logging.debug(
            f"InMemoryCache.set({range}, <{len(data)} bytes>, <{sum(len(v) for v in self.cache.values())/1024**2:.1f} MB total size>)"
        )
...
class HttpFile:
    ...
    def __init__(
        ...
        self.content = None
        logging.debug(
            f"HttpFile initialized: url={url}, block_size={block_size}, extra_headers={self.extra_headers}"
        )
    ...
    def _get_length_from_head(self) -> int | None:
        ...
        """
        logging.debug(
            f"HttpFile._get_length_from_head: Sending HEAD request to {self.url}"
        )
        try:
            response = self.session.head(
                self.url, headers=self.extra_headers, allow_redirects=True
            )
            response.raise_for_status()
            length = response.headers.get(CONTENT_LENGTH_HEADER)
            if length is not None and int(length) > 0:
                logging.debug(
                    f"HttpFile._get_length_from_head: Content-Length from HEAD={length}"
                )
                return int(length)
            return None
    ...
    def _parse_content_range(self, content_range: str) -> int | None:
        ...
        try:
            ...
            logging.debug(
                f"HttpFile._parse_content_range: Parsed total from Content-Range: {total}"
            )
            return int(total)
        except (ValueError, IndexError):
            logging.debug(
                f"HttpFile._parse_content_range: Failed to parse Content-Range: {content_range}"
            )
            return None
    ...
    def _get_length_from_range_request(self) -> int | None:
        ...
        """
        logging.debug(
            "HttpFile._get_length_from_range_request: Trying GET with Range: bytes=0-0"
        )
        ...
        try:
            ...
            if length is not None and int(length) > 0:
                logging.debug(
                    f"HttpFile._get_length_from_range_request: Content-Length from GET={length}"
                )
                return int(length)
            ...
            logging.debug(
                f"HttpFile._get_length_from_range_request: GET request failed: {e}"
            )
            raise FileLengthError(f"Failed to get file length: {e}")
    ...
    def file_length(self) -> int:
        ...
        logging.debug(
            "HttpFile.file_length: Could not determine file length from HEAD or GET"
        )
        raise FileLengthError(
            "Content-Length header is missing and could not be determined via GET"
        )
    ...
    def _get_block_data(self, block_range: tuple[int, int]) -> bytes:
        ...
        logging.debug(
            f"HttpFile._get_block_data: Cache MISS for block_range={block_range}, sending HTTP GET"
        )
        headers = self.extra_headers.copy()
        ...
        try:
            ...
            logging.debug(
                f"HttpFile._get_block_data: Received {len(block_data)} bytes for block_range={block_range}"
            )
            return block_data
        except RequestException as e:
            logging.debug(
                f"HttpFile._get_block_data: HTTP GET failed for block_range={block_range}: {e}"
            )
            raise DataFetchError(f"Failed to fetch data: {e}")

    def _extract_relevant_data(
        self, block_data: bytes, block_start: int, current_offset: int, end_offset: int
    ) -> bytes:
    ...
    def seek(self, offset: int, whence: int = io.SEEK_SET) -> None:
        ...
        if whence == io.SEEK_CUR:
            ...
            self.offset = offset
        logging.debug(
            f"HttpFile.seek(offset={offset}, whence={whence}): {old_offset} -> {self.offset}"
        )
    ...
    def __exit__(
        ...
        logging.debug(
            f"HttpFile.__exit__() called. exc_type={exc_type}, exc_value={exc_value}"
        )
        self.close()
...
if __name__ == "__main__":
    ...
    with HttpFile(
        ...
        data: bytes = f.read(
            8
        )  # Sends the GET request for a full block and returns the first 8 bytes
        print(data)
