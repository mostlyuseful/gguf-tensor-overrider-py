    ...
    def test_get_gpu_configurations_validation_errors(self):
        ...
        with pytest.raises(ValueError, match="Cannot use both"):
            self.manager.get_gpu_configurations(
                use_system_gpus=True, gpu_vram_config="0=6"
            )

        # Neither option specified
        with pytest.raises(ValueError, match="Must specify either"):
            self.manager.get_gpu_configurations(
                use_system_gpus=False, gpu_vram_config=None
            )
...
class TestTensorAllocator:
    ...
    def test_allocate_tensors_simple(self):
        ...
        # Create test data
        block_groups = [BlockGroup(0), BlockGroup(1)]
        ...
        total_allocated = sum(
            gpu.allocated_bytes for gpu in result.gpu_allocations.values()
        )
    ...
    def test_allocate_tensors_overflow(self):
        ...
        huge_tensor = TensorInfo(
            "huge_global.weight", 10 * 1024**3
        )  # 10GB, no block pattern
...
class TestGenericOutputFormatter:
    ...
    def test_format_allocation(self):
        ...
        result.tensor_gpu_mapping = {"tensor1": 0, "tensor2": 0, "tensor3": 1}
