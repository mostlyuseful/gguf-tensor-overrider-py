...
class TestGPUCapacity:
    ...
    def test_initialization(self):
        ...
        gpu = GPUCapacity(
            gpu_id=0, total_vram_bytes=8 * 1024**3, usable_percentage=80.0
        )
    ...
    def test_usable_vram_calculation(self):
        ...
        gpu = GPUCapacity(
            gpu_id=0, total_vram_bytes=10 * 1024**3, usable_percentage=90.0
        )
    ...
    def test_available_bytes_calculation(self):
        ...
        gpu = GPUCapacity(
            gpu_id=0, total_vram_bytes=8 * 1024**3, usable_percentage=100.0
        )
    ...
    def test_utilization_percentage(self):
        ...
        gpu = GPUCapacity(
            gpu_id=0, total_vram_bytes=10 * 1024**3, usable_percentage=80.0
        )
    ...
    def test_can_fit_tensor(self):
        ...
        gpu = GPUCapacity(
            gpu_id=0, total_vram_bytes=4 * 1024**3, usable_percentage=100.0
        )
        gpu.kv_cache_reserved_bytes = 1 * 1024**3
    ...
    def test_allocate_tensor_failure(self):
        ...
        gpu = GPUCapacity(
            gpu_id=0, total_vram_bytes=1 * 1024**3, usable_percentage=100.0
        )
...
class TestKVCacheConfig:
    ...
    def test_bytes_per_layer(self):
        ...
        config = KVCacheConfig(
            context_length=2048, k_dtype=DataType.F16, v_dtype=DataType.F16
        )
        ...
        expected = (
            2048 * 32 * 128 * (2 + 2)
        )  # context * kv_heads * head_dim * (k_bytes + v_bytes)
        assert config.bytes_per_layer(metadata) == expected

    def test_total_bytes(self):
        """Test total KV cache calculation."""
        config = KVCacheConfig(
            context_length=1024, k_dtype=DataType.F16, v_dtype=DataType.F16
        )
...
class TestAllocationResult:
    ...
    def test_total_calculations(self):
        ...
        gpu1 = GPUCapacity(
            gpu_id=1, total_vram_bytes=8 * 1024**3, usable_percentage=100.0
        )
    ...
    def test_allocation_summary(self):
        ...
        gpu = GPUCapacity(
            gpu_id=0, total_vram_bytes=4 * 1024**3, usable_percentage=80.0
        )
        ...
        assert gpu_info["utilization_percent"] == pytest.approx(
            62.5
        )  # 2GB used / 3.2GB usable
        # New fields
        ...
        assert gpu_info["utilization_percent_of_total"] == pytest.approx(
            50.0
        )  # 2GB used / 4GB total
    ...
    def test_utilization_respects_total_limit(self):
        ...
        gpu = GPUCapacity(
            gpu_id=3, total_vram_bytes=24 * 1024**3, usable_percentage=90.0
        )
        ...
        assert (
            gpu_info["utilization_percent_of_total"] <= gpu_info["usable_percent_limit"]
        )
